...
from models import db, User, Score, Quiz, Subject, UserBadge, UserLessonProgress
from datetime import datetime, timedelta

parent_bp = Blueprint('parent', __name__)


def get_child_report_data(user):
...
def user_report(user_id):
    ...
    data = get_child_report_data(user)
    return render_template('parent_user_report.html', **data)
...
def email_report(user_id):
    """Email a HTML report to the parent."""
    if not session.get('is_parent'):
        flash('Please log in as a parent first.', 'warning')
        return redirect(url_for('parent.login'))
    ...
    try:
        ...
        send_report_email(parent_email, user.username, data)
