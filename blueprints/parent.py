...
@parent_bp.route('/user/<int:user_id>/download_pdf')
def download_pdf(user_id):
    """Download a PDF report for a child."""
    if not session.get('is_parent'):
        flash('Please log in as a parent first.', 'warning')
        return redirect(url_for('parent.login'))
    ...
    user_data = {
        'user': user, 'total_quizzes': total_quizzes, 'avg_score': avg_score,
        'subject_progress': subject_progress, 'badges': badges,
        'recent_activity': recent_activity, 'progress_to_next': progress_to_next,
        'lessons_completed': lessons_completed, 'pdf_mode': True
    }

    try:
        from report_generator import generate_pdf
        pdf_data = generate_pdf(user_data)
        ...
        return redirect(url_for('parent.user_report', user_id=user.id))


@parent_bp.route('/user/<int:user_id>/email_report', methods=['POST'])
def email_report(user_id):
    """Email a PDF report to the parent."""
    if not session.get('is_parent'):
        flash('Please log in as a parent first.', 'warning')
        return redirect(url_for('parent.login'))
    ...
    if not user.parent_email:
        flash('No parent email on file for this child.', 'error')
        return redirect(url_for('parent.dashboard'))
    ...
    try:
        from report_generator import generate_pdf, send_report_email
        pdf_data = generate_pdf(user_data)
        send_report_email(parent_email, user.username, pdf_data)
        flash(f'Report sent to {parent_email}!', 'success')
    except ValueError as e:
        flash(str(e), 'error')
    except Exception as e:
        flash(f'Could not send email: {e}. Check SMTP settings.', 'error')
